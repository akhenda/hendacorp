const TEST_FUNCTION_NAMES = ['it', 'xit', 'fit', 'test', 'xtest', 'describe', 'fdescribe', 'xdescribe'];
...
function isTestDefinition(node) {
  return node.type === 'CallExpression' && TEST_DEFINITION_NAMES.includes(getTestMethodName(node));
}
