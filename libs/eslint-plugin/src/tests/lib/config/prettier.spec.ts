...
describe('config', () => {
  describe('prettier', () => {
    it('validates source files using prettier', () => {
      expect(execESLint(`--config ${configFile('prettier')} ${fixtureFile('prettier')}`)).toMatch(
    ...
    it('validates TypeScript source files using prettier', () => {
      expect(
        execESLint(`--config ${fixtureFile('prettier-typescript/.eslintrc.js')} ${fixtureFile('prettier-typescript')}`),
